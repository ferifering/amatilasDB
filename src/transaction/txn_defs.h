...
/* 事务回滚原因 */
enum class AbortReason { LOCK_ON_SHIRINKING = 0, UPGRADE_CONFLICT, DEADLOCK_PREVENTION, ATTEMPTED_INTENTION_LOCK_ON_RECORD_LOCKED_BY_OTHER_TXN };
   ...
   public:
    ...
    std::string GetInfo() {
        switch (abort_reason_) {
            ...
            case AbortReason::ATTEMPTED_INTENTION_LOCK_ON_RECORD_LOCKED_BY_OTHER_TXN: {
                return "Transaction " + std::to_string(txn_id_) + " aborted because attempted intention lock on record locked by other transaction\n";
            } break;
