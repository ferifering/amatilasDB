...
// 为std::pair<std::string, Rid>定义哈希函数
namespace std {
template <> struct hash<std::pair<std::string, Rid>> {
  std::size_t operator()(const std::pair<std::string, Rid> &p) const {
    auto h1 = std::hash<std::string>{}(p.first);
    auto h2 = std::hash<int>{}(p.second.page_no);
    auto h3 = std::hash<int>{}(p.second.slot_no);
    return h1 ^ (h2 << 1) ^ (h3 << 2);
  }
};
} // namespace std
...
struct VersionUndoLink {
  ...
  inline static std::optional<VersionUndoLink>
  FromOptionalUndoLink(std::optional<UndoLink> undo_link) {
...
public:
   ...
   */
  void UpdateUndoLink(Rid rid, std::optional<UndoLink> undo_link,
   ...
   * 在更新之前，将调用 `check` 函数以确保有效性。
   */
  void UpdateVersionLink(Rid rid, std::optional<VersionUndoLink> version_link,
                         const std::string &table_name);

  /** @brief 获取表堆元组的第一个撤销日志。 */
  std::optional<UndoLink> GetUndoLink(Rid rid, const std::string &table_name);

  /** @brief 获取表堆元组的第一个撤销日志。*/
  std::optional<VersionUndoLink> GetVersionLink(Rid rid,
                                                const std::string &table_name);

  /** @brief 访问事务撤销日志缓冲区并获取撤销日志。如果事务不存在，返回
   * nullopt。 如果索引超出范围仍然会抛出异常。 */
  std::optional<UndoLog> GetUndoLogOptional(UndoLink link);

  /** @brief 访问事务撤销日志缓冲区并获取撤销日志。除非访问当前事务缓冲区，
   * 否则应该始终调用此函数以获取撤销日志，而不是手动检索事务 shared_ptr
   * 并访问缓冲区。 */
  ...
  struct PageVersionInfo {
    std::optional<UndoLink> undo_link_;
    std::optional<VersionUndoLink> version_link_;
  };

  /** 保护版本信息 */
  std::shared_mutex page_version_info_mutex_;
  /** 存储表堆中每个元组的先前版本。键为(table_name, Rid) */
  std::unordered_map<std::pair<std::string, Rid>, PageVersionInfo>
      page_version_info_;
...
private:
  ...
  Watermark watermark_{0};                     // 水印管理器，用于MVCC垃圾回收
  std::unordered_set<txn_id_t>
      running_txns_; // 存储所有正在运行的事务ID，仅用于MVCC
};