...
auto ql_manager = std::make_unique<QlManager>(sm_manager.get(), txn_manager.get(), nullptr, buffer_pool_manager.get());
...
void *client_handler(void *sock_fd)
{
    ...
    while (true)
    {
        ...
        {
            // 解析失败，输出failure到output.txt文件中
            std::string error_msg = "Parser Error";
            memcpy(data_send, error_msg.c_str(), error_msg.length());
            data_send[error_msg.length()] = '\n';
            data_send[error_msg.length() + 1] = '\0';
            offset = error_msg.length() + 1;
    ...
    if (txn_id != INVALID_TXN_ID) {
        ...
        if (txn != nullptr && txn->get_state() == TransactionState::GROWING) {
            std::cout << "Rolling back uncommitted transaction " << txn_id << " on client disconnect" << std::endl;
...
int main(int argc, char **argv)
{
    ...
    try
    {
        ...
        // recovery database using checkpoint-based recovery
