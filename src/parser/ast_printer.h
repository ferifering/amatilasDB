...
private:
    ...
    static void print_node(const std::shared_ptr<TreeNode> &node, int offset) {
        ...
        if (auto x = std::dynamic_pointer_cast<Help>(node)) {
            ...
            print_val(x->col_name, offset);
            if (!x->alias.empty()) {
                print_val("ALIAS: " + x->alias, offset);
            }
            if (x->isAgg) {
                print_val("AGG_TYPE: " + std::to_string(static_cast<int>(x->aggType)), offset);
            }
            ...
            print_node_list(x->conds, offset);
            if (x->has_group_by && x->group_by_clause) {
                std::cout << offset2string(offset) << "GROUP BY\n";
                print_node_list(x->group_by_clause->group_by_cols, offset + 2);
            }
            if (x->has_having && x->having_clause) {
                std::cout << offset2string(offset) << "HAVING\n";
                print_node_list(x->having_clause->conds, offset + 2);
            }
