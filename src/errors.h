...
class IncompatibleCountError : public RMDBError {
public:
    IncompatibleCountError(const std::string s)
            : RMDBError(s) {}
};
...
public:
    PageNotExistError(const std::string &table_name, int page_no)
        : RMDBError("Page " + std::to_string(page_no) + " in table " + table_name + " does not exist") {}
};