...
{
    public:
        SortPlan(std::shared_ptr<Plan> subplan, std::vector<TabCol> sel_cols, std::vector<bool> is_desc_list)
        {
            Plan::tag = T_Sort;
            subplan_ = std::move(subplan);
            sel_cols_ = std::move(sel_cols);
...
{
    public:
        LimitPlan(std::shared_ptr<Plan> subplan, int limit_count)
        {
            Plan::tag = T_Projection; // 使用现有的tag或添加新的T_Limit
            subplan_ = std::move(subplan);
...
public:
    std::shared_ptr<Plan> prev_;
    std::vector<std::shared_ptr<ast::Col>> agg_exprs_;
    std::vector<std::shared_ptr<ast::Col>> group_by_cols_;
    std::vector<std::shared_ptr<ast::BinaryExpr>> having_conds_;
    std::vector<std::shared_ptr<ast::Col>> select_cols_;
    bool has_group_by_;
    bool has_having_;
    
    AggregatePlan(std::shared_ptr<Plan> prev,
