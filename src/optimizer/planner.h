...
class Planner {
   ...
   private:
    ...
    std::shared_ptr<Plan> make_one_rel_with_jointree(std::shared_ptr<Query> query);

    std::shared_ptr<Plan> generate_select_plan(std::shared_ptr<Query> query, Context *context);

    // 逻辑优化函数
    std::shared_ptr<Query> optimize_predicate_pushdown(std::shared_ptr<Query> query, Context *context);
    std::shared_ptr<Query> optimize_constant_folding(std::shared_ptr<Query> query, Context *context);
    std::shared_ptr<Query> optimize_redundant_conditions(std::shared_ptr<Query> query, Context *context);
    std::shared_ptr<Query> optimize_projection_pushdown(std::shared_ptr<Query> query, Context *context);
    std::shared_ptr<Query> optimize_join_reordering(std::shared_ptr<Query> query, Context *context);

    // int get_indexNo(std::string tab_name, std::vector<Condition> curr_conds);
    bool get_index_cols(std::string tab_name, std::vector<Condition> curr_conds, std::vector<Condition>& ret_conds, std::vector<std::string>& index_col_names);
